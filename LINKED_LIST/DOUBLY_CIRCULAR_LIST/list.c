...
len_t get_list_length(list_t* p_list)
{
    ...
    for(run = p_list->next; run != p_list; run = run->next, len++);
...
list_t* get_reversal_list(list_t* p_list)
{
    ...
    while(p_run != p_list)
    {
        assert(insert_end(p_reversed_list, p_run->data));
        p_run = p_run->prev;
    }
...
status_t reverse_list(list_t* p_list)
{
    ...
    p_current_last = p_list->prev;
    p_original_prev = p_current_last;
    p_run = p_current_last->prev;
...
static void* xcalloc(size_t nr_elements, size_t size_per_element)
{
    ...
    if(NULL == p)
    {
        fprintf(stderr, "fatal: memory not allocated\n");
        exit(EXIT_FAILURE);
    }
